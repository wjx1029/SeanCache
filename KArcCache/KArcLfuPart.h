...
public:
    ...
    bool put(Key key, Value value)
    {
        ...
        if (contain(key))
        {
            ...
            return updateExistingNode(it->second);
        }
...
private:
    ...
    void evictLeastFrequent() 
    {
        ...
        // 移除最小频率的列表的第一个节点
        NodePtr leastFreqNode = minFreqList.front();
        ...
        addToGhost(leastFreqNode);
        
        // 从主缓存删除
        mainCahce_.erase(leastFreqNode->getKey());
    }
    ...
    void addToGhost(NodePtr node)
    {
        ...
        node->prev_ = ghostTail->prev_;
        if (!ghostTail->prev_.expired())
        {
            ghostTail_->prev.lock()->next_ = node;
        }
